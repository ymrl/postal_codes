...
export const BodyRow = ({
  ...
  return (
    <Row rowIndex={rowIndex} type="body">
      {/*
    ...
    */}
      {/* 郵便番号（7桁） */}
      <Cell columnType="number" colIndex={1}>
        <NumberCellContent>
          {row[2].slice(0, 3)}-{row[2].slice(3)}
        </NumberCellContent>
      </Cell>
      {/* 都道府県名 */}
      <Cell columnType="pref" colIndex={2}>
        <PrefCellContent kanji={row[6]} kana={row[3]} />
      </Cell>
      {/* 市区町村名 */}
      <Cell columnType="city" colIndex={3}>
        <CityCellContent kanji={row[7]} kana={row[4]} />
      </Cell>
      {/* 町域名 */}
      <Cell columnType="town" colIndex={4}>
        <TownCellContent kanji={row[8]} kana={row[5]} />
      </Cell>
      <Cell columnType="others" colIndex={5}>
        <Others
          list={[
